 ...
 */
export class TodosMcpTool {
  constructor(private db: DatabaseContext) {}

  /**
   * Creates a new todo with a description and creation date.
   */
  async createTodoAsync(description: string, createdDate: Date): Promise<string> {
    try {
   ...
   */
  async readTodosAsync(id?: string): Promise<Todo[]> {
    try {
      if (id && id.trim() !== '') {
   ...
   */
  async updateTodoAsync(id: string, description?: string, createdDate?: Date): Promise<string> {
    try {
      const todoId = parseInt(id, 10);
      if (isNaN(todoId)) {
   ...
   */
  async deleteTodoAsync(id: string): Promise<string> {
    try {
      const todoId = parseInt(id, 10);
      if (isNaN(todoId)) {
        return 'Invalid todo id.';
